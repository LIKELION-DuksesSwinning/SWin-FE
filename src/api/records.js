...
export const createSwimSchedule = async (dateString) => {
  ...
  const response = await fetch('https://miseno.store/api/v1/schedules/', {
    method: 'POST',
    headers: {
...
export const getSwimRecords = async (sort = 'latest') => {
  return apiRequest(`/api/v1/records/swim/?sort=${encodeURIComponent(sort)}`);
};

export const createSwimRecord = async ({ timing, date, startTime, durationMinutes, photo, schedule, swimTime, symptoms, memo }) => {
  ...
  if (photo) {
    const compressedPhoto = await compressImage(photo, { maxWidth: 800, maxHeight: 800, quality: 0.7, maxFileSize: 700 * 1024 });
  ...
  if (Array.isArray(symptoms)) {
    formData.append('symptoms', stringifySymptoms(symptoms));
  }
...
export const getSwimRecord = async (recordId) => {
  if (!recordId) throw new Error('record_id가 필요합니다.');
  return apiRequest(`/api/v1/records/swim/${encodeURIComponent(recordId)}/`);
};

export const updateSwimRecord = async (recordId, { photo, swimTime, symptoms, memo } = {}) => {
  if (!recordId) throw new Error('record_id가 필요합니다.');
  const formData = new FormData();
...
export const createAdditionalRecord = async ({ recordId, photo, symptoms, memo }) => {
  if (!recordId) throw new Error('record_id가 필요합니다.');
  const formData = new FormData();

  if (photo) {
    const compressedPhoto = await compressImage(photo, { maxWidth: 200, maxHeight: 200, quality: 0.7, maxFileSize: 700 * 1024 });
    formData.append('photo', compressedPhoto, compressedPhoto.name);
  }
  ...
  return apiRequest(`/api/v1/records/swim/${encodeURIComponent(recordId)}/additional/`, {
    method: 'POST',
    body: formData,
    isFormData: true,
  });
